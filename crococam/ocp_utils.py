...
def plot_ocp_state(ddp_data, fig=None, ax=None, label=None, marker=None, color=None, alpha=1., MAKE_LEGEND=False, SHOW=True, x_limits_lower=None, x_limits_upper=None):
    ...
    if(ax is None or fig is None):
        ...
        fig.canvas.manager.set_window_title('ocp_state')
    if(label is None):
        label='State'
    for i in range(nq):
        # Plot positions
        ...
        if('stateReg' in ddp_data['active_costs']):
            ...
            if('reg_ref' in labels):
                handles.pop(labels.index('reg_ref'))
                ax[i,1].lines[labels.index('reg_ref')].remove()
                labels.remove('reg_ref')
...
def plot_ocp_control(ddp_data, fig=None, ax=None, label=None, marker=None, color=None, alpha=1., MAKE_LEGEND=False, SHOW=True):
    ...
    if(ax is None or fig is None):
        ...
        fig.canvas.manager.set_window_title('ocp_control')
    if(label is None):
        label='Control'    
...
def plot_ocp_endeff_linear(ddp_data, fig=None, ax=None, label=None, marker=None, color=None, alpha=1., 
    ...
    if(ax is None or fig is None):
        ...
        fig.canvas.manager.set_window_title('ocp_endeff_linear')
    if(label is None):
        label='OCP solution'
    xyz = ['x', 'y', 'z']
    for i in range(3):
...
def plot_ocp_endeff_angular(ddp_data, fig=None, ax=None, label=None, marker=None, color=None, alpha=1., 
    ...
    if(ax is None or fig is None):
        fig, ax = plt.subplots(3, 2, sharex='col')
        fig.canvas.manager.set_window_title('ocp_endeff_angular')
    if(label is None):
        label='OCP solution'
    xyz = ['x', 'y', 'z']
    for i in range(3):
...
def plot_ocp_force(ddp_data, fig=None, ax=None, label=None, marker=None, color=None, alpha=1., 
    ...
    if(ax is None or fig is None):
        fig, ax = plt.subplots(3, 2, sharex='col')
        fig.canvas.manager.set_window_title('ocp_force')
    if(label is None):
